...
FAST MODE: 2-step filter — fetches all NSE stocks but only deep-scans
top 500 by liquidity. Runs in under 8 minutes.
"""
...
MAX_STOCKS   = 500   # Max stocks to deep-scan
MIN_PRICE    = 50    # Minimum CMP in ₹ (filter penny stocks)

def hdrs():
    return {"Content-Type":"application/json","access-token":ACCESS_TOKEN,"client-id":CLIENT_ID}

# ── FETCH ALL NSE EQ STOCKS FROM DHAN MASTER ─────────────
def fetch_master():
    print("  Fetching Dhan NSE_EQ instrument master...")
    try:
        ...
        i_sym = col('sem_trading_symbol','trading_symbol','symbol','tradingsymbol')
        i_sid = col('sem_smst_security_id','security_id','securityid','isin')
        i_ser = col('sem_series','series','instrument_type','series_desc')
        i_nam = col('sem_custom_symbol','custom_symbol','company_name','name','description')

        print(f"  Col indices → sym:{i_sym} sid:{i_sid} series:{i_ser}")
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                for k,v in raw.items():
                    p = v.get("last_price") if isinstance(v,dict) else v
                    if p: result[str(k)] = float(p)
        except: pass
        time.sleep(0.3)
    return result
...
def fetch_index_ids():
    try:
        ...
        def col(*n):
            ...
            return -1
        i_sym = col('sem_trading_symbol','trading_symbol','symbol')
        i_sid = col('sem_smst_security_id','security_id','securityid')
        if i_sym==-1 or i_sid==-1: return {}
        idx={}
        for row in reader:
            if len(row)<=max(i_sym,i_sid): continue
            s=row[i_sym].strip().upper(); d=row[i_sid].strip()
            if s and d: idx[s]=d
        print(f"  Index keys (sample): {list(idx.keys())[:8]}")
        return idx
    except Exception as e:
...
def dhan_hist(sec_id, seg="NSE_EQ", days=280, instrument="EQUITY"):
    ...
    try:
        r = requests.post(f"{BASE}/charts/historical",headers=hdrs(),json=payload,timeout=20)
        if r.status_code==200:
            d=r.json()
            if isinstance(d,dict) and "close" in d and len(d["close"])>0: return d
    except: pass
    return None
...
def market_direction(nh,vix=None):
    if not nh or "close" not in nh or len(nh["close"])<200:
        return {"regime":"UNKNOWN","exposure":75,"cmp":0,"s50":0,"s150":0,"s200":0,
                "r1m":0,"r3m":0,"vix":vix,"signals":["Nifty data unavailable"]}
...
def build_html(results,mkt,sectors,scan_time,total_master,total_filtered):
    ...
    for s in sectors[:9]:
        ...
        sh+=(f'<div style="background:{bg};border:1px solid {col}33;border-radius:8px;padding:10px 12px">'
             ...
             f'<div style="font-family:monospace;font-size:10px;color:#94a3b8">1M:{s["r1m"]:+.1f}% 3M:{s["r3m"]:+.1f}%</div></div>')
    rows=""
    for i,s in enumerate(results):
        ...
        rows+=(f'<tr style="border-bottom:1px solid rgba(255,255,255,.05)">'
               ...
               f'<td style="padding:7px 8px;font-family:monospace;font-size:10px">₹{s["posval"]:,.0f}</td></tr>')
    ...
    stats=[(total_master,"NSE master","#94a3b8"),(total_filtered,f"Scanned (₹{MIN_PRICE}+)","#e2e8f0"),
           (len(results),"Qualified","#e2e8f0"),(tt7,"TT 7-8","#22c55e"),
           (vcp_c,"VCP","#f59e0b"),(ap,"A+ Grade","#a78bfa"),(a_c,"A Grade","#3b82f6")]
    sthtml="".join(f'<div style="padding:12px 14px;border-right:1px solid rgba(255,255,255,.07)"><div style="font-family:monospace;font-size:19px;font-weight:600;color:{c}">{v}</div><div style="font-size:9px;color:#64748b;margin-top:2px;text-transform:uppercase;letter-spacing:.05em">{l}</div></div>' for v,l,c in stats)
...
<div style="padding:8px 20px;border-top:1px solid rgba(255,255,255,.07);display:flex;justify-content:space-between;font-size:9px;color:#64748b;font-family:monospace;background:#0f1525">
  <span>Dhan API · NSE Liquid Universe (₹{MIN_PRICE}+ stocks) · Minervini SEPA · Auto-generated daily</span>
...
def main():
    print("="*60)
    print("  GAURAV'S TRADING SYSTEM v6 — FAST MODE")
    print(f"  {datetime.datetime.now().strftime('%Y-%m-%d %H:%M IST')}")
    print("="*60)
    if not ACCESS_TOKEN:
        print("ERROR: DHAN_ACCESS_TOKEN not set"); sys.exit(1)
    print(f"  Token: {len(ACCESS_TOKEN)} chars | Client: {CLIENT_ID}")

    # Step 0: Fetch master
    print("\n[0/3] Fetching NSE instrument master...")
    all_stocks = fetch_master()
    if not all_stocks:
        print("CRITICAL: Cannot fetch master. Exiting."); sys.exit(1)
    print(f"  Total EQ stocks: {len(all_stocks)}")

    # Step 1: Index data
    print("\n[1/3] Fetching index & VIX data...")
    idx_ids = fetch_index_ids()
    def get_idx_sid(*keys):
        for k in keys:
            if idx_ids.get(k): return idx_ids[k]
        return None

    nifty_sid = get_idx_sid("NIFTY 50","NIFTY50","NIFTY_50")
    print(f"  Nifty SID: {nifty_sid}")
    nh = None
    if nifty_sid:
    ...
    print(f"  Nifty hist: {'OK '+str(len(nh.get('close',[])))+'c' if nh else 'FAILED'}")

    vix_sid = get_idx_sid("INDIA VIX","INDIAVIX","INDIA_VIX")
    ...
    idx_hist = {}
    for key, label in [("NIFTY BANK","BANKNIFTY"),("NIFTYBANK","BANKNIFTY"),
    ...
    # Step 2: FAST FILTER — batch LTP, keep only stocks ≥ MIN_PRICE
    print(f"\n[2/3] Fast filter — fetching LTP for {len(all_stocks)} stocks...")
    sid_list = [s['sid'] for s in all_stocks]
    ltp_map  = batch_ltp(sid_list)
    print(f"  Got LTP for {len(ltp_map)} stocks")

    # Filter: price >= MIN_PRICE, sort by price descending, take top MAX_STOCKS
    liquid = []
    for s in all_stocks:
        p = ltp_map.get(str(s['sid']))
        if p and p >= MIN_PRICE:
            s['ltp'] = p
            liquid.append(s)

    # Sort by price descending (higher price = more likely institutional quality)
    liquid.sort(key=lambda x: x['ltp'], reverse=True)
    to_scan = liquid[:MAX_STOCKS]
    print(f"  Stocks ≥ ₹{MIN_PRICE}: {len(liquid)} | Scanning top {len(to_scan)} by price")

    # Step 3: Deep scan — historical + Minervini analysis
    print(f"\n[3/3] Deep scanning {len(to_scan)} stocks...")
    results = []
    for i, stock in enumerate(to_scan):
        ...
        pct_done = (i+1)/len(to_scan)*100
        print(f"  [{i+1:>3}/{len(to_scan)}] {sym:<16} {pct_done:.0f}% | OK:{len(results)}", end="\r")
        time.sleep(0.05)  # Faster — 50ms per stock = 500 stocks in ~25s
        if (i+1) % 100 == 0: time.sleep(1)

    scan_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M IST")
    print(f"\n\n{'='*60}")
    print(f"  NSE master: {len(all_stocks)} | Filtered: {len(to_scan)} | Qualified: {len(results)}")
    print(f"  TT 7-8: {sum(1 for r in results if r['tts']>=7)}")
    print(f"  A+: {sum(1 for r in results if r['grade']=='A+')} | VCP: {sum(1 for r in results if r['vcp'])}")
    if results:
    ...
    html = build_html(results, mkt, sectors, scan_time, len(all_stocks), len(to_scan))
